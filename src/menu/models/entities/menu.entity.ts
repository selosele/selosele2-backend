...
export class MenuEntity extends BaseEntity {
  ...
  @Column({ comment: '메뉴 링크' })
  @ApiProperty({ description: '메뉴 링크' })
  link?: string;

  @Column({ comment: '메뉴 정렬 순서' })
  @ApiProperty({ description: '메뉴 정렬 순서' })
  sort?: number;

  @Column({ comment: '메뉴 계층' })
  ...
  @CreateDateColumn({ comment: '메뉴 등록일시' })
  @ApiProperty({ description: '메뉴 등록일시' })
  regDate?: Date;

  @UpdateDateColumn({ comment: '메뉴 수정일시' })
